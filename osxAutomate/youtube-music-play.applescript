to clickClassName(theClassName, elementnum) -- Handler for pausing YouTube in Chrome
	if application "Vivaldi" is running then
		try
			tell application "Vivaldi" to (tabs of window 1 whose URL contains "music.youtube")
		...
		try
			tell application "Google Chrome" to (tabs of window 1 whose URL contains "music.youtube")
			set youtubeTabs to item 1 of the result
		...
		try
			tell application "Google Chrome" to (tabs of window 2 whose URL contains "music.youtube")
			set youtubeTabs to item 1 of the result
			tell application "Google Chrome"
				execute youtubeTabs javascript "document.getElementsByClassName('" & theClassName & "')[" & elementnum & "].click();"
			end tell
...
clickClassName("play-pause-button", 0)